...
/* Includes ---------------------------------------------------------------- */
#include <Conveyor_Belt_inferencing.h>
...
void setup(){
    ...
    // Ergenzungen (steuerung und LED)
    pinMode(LED_BUILTIN, OUTPUT);
    pinMode(CONVEYOR_CONTROL_PIN,OUTPUT);
    digitalWrite(LED_BUILTIN, LOW); // werte aus irgend einem grund verdreht (durch zuffal herausgefunden)
    digitalWrite(CONVEYOR_CONTROL_PIN,HIGH);
    delay(3000); // 3 sekunden test (band fährt)
    digitalWrite(LED_BUILTIN, HIGH);
    digitalWrite(CONVEYOR_CONTROL_PIN, LOW);
}
...
void loop()
{ 
    ei_printf("\nStarting inferencing in 0.15 seconds...\n");

    // instead of wait_ms, we'll wait on the signal, this allows threads to cancel us...
    if (ei_sleep(150) != EI_IMPULSE_OK) {
        return;
    }
    ...
    for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
        ...
        // code lampe an wenn grün erkannt wird 
        // fliesband aus wenn erkannt wird
        if (strcmp(ei_classifier_inferencing_categories[i], "qudrat_gruen") == 0 ||strcmp(ei_classifier_inferencing_categories[i], "quadrat_blau") == 0 || strcmp(ei_classifier_inferencing_categories[i], "qudrat_rot") == 0  ) {
            if (result.classification[i].value > 0.6) { // Falls Wahrscheinlichkeit hoch genug ist
                digitalWrite(LED_BUILTIN, LOW);  // LED einschalten 
                digitalWrite(CONVEYOR_CONTROL_PIN,LOW);
                delay(2000);  // Kurz pausieren
                break;
            } else {
